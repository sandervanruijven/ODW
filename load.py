...
for i in range(0, len(df)):
    try:
        folium.Marker([df.iloc[i]['Y'], df.iloc[i]['X']],
                        ...
                        tooltip = f"<b>Fout: {df.iloc[i]['fout']}</b><br>Beeld: {df.iloc[i]['beeld']}<br>MSI-type: {df.iloc[i]['msi_type']}<br>Locatie: {df.iloc[i]['wegsoort']} {df.iloc[i]['Wegnummer']} {df.iloc[i]['hm']} {df.iloc[i]['baansoort']} {df.iloc[i]['rijrichting']} {int(df.iloc[i]['strooknummer_new'])}<br>Duur storing: {df.iloc[i]['duur']}<br>Duur storing: {df.iloc[i]['duur']}<br>Categorie storing: {df.iloc[i]['categorie']}",
