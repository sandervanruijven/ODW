...
for i in range(0, len(df)):
    try:
        folium.Marker([df.iloc[i]['Y'], df.iloc[i]['X']],
                        popup = folium.Popup(f"<b>{df.iloc[i]['asset']}</b>", max_width=300, min_width=150),
                        tooltip = f"<b>Fout: {df.iloc[i]['fout']}</b><br>Beeld: {df.iloc[i]['beeld']}<br>MSI-type: {df.iloc[i]['msi_type']}<br>Locatie: {df.iloc[i]['Wegnummer']}<br>Duur storing: {df.iloc[i]['duur']}<br>Categorie storing: {df.iloc[i]['categorie']}",
